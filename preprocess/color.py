...
if(args.type == 0):
    img = Image.open(args.person_path + "/" + args.person)  # 读取照片
    x,y = img.size
    p = Image.new('RGBA',img.size,(255,255,255))
    p.paste(img,(0,0,x,y),img)
    p = p.convert('RGB')
    p.save(args.person_path + "/" + args.person)

    print('Person Convert To White !')

elif(args.type == 1):
    img = Image.open(args.cloth_path + "/" + args.cloth)  # 读取照片
    ...
    img = img.convert('L')
    img.save(args.cloth_path + "/" + args.cloth)  # 保存图片
