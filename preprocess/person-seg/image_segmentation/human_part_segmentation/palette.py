...
img0 = Image.open('./output' + "/" + args.person + '.png')
...
img0.save('./output' + "/" + args.person + '.png')


# -------------- add neck by brown color -------------- #
small_face, big_face, real_face = [], [], []      # real = big - small
no_neck_img = Image.open('./output'  + "/" + args.person+ '1.png')         # pixel 13 means face
...
for i in range(width):
    for n in range(height):
        if load0[i, n] != 0:
            load0[i, n] = 255
img0.save('./output'  + "/" + args.person+ '1.png')
